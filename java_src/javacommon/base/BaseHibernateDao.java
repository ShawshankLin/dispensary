...
public abstract class BaseHibernateDao<E,PK extends Serializable> extends HibernateDaoSupport implements EntityDao<E,PK>{
    ...
    public List<E> findAllBy(String sql,final PageRequest pageRequest) {
    	final String countQuery = "select count(*) " + removeSelect(removeFetchKeyword((sql)));
    	Map otherFilters = new HashMap(1);
		otherFilters.put("sortColumns", pageRequest.getSortColumns());
		...
		//混合使用otherFilters与pageRequest为一个filters使用
		final XsqlFilterResult queryXsqlResult = builder.generateHql(sql,otherFilters,pageRequest);
		return getHibernateTemplate().execute(new HibernateCallback<List>() {
			public List doInHibernate(Session session) throws HibernateException, SQLException {
				
				Query query = PageQueryUtils.setQueryParameters(session.createQuery(queryXsqlResult.getXsql()),pageRequest);
				return query.list();
			}
		});
	}
	...
	public void saveAllBatch(Collection<E> list){
		int i=0;
